...
describe('the scrub covers every path both collections carry', () => {
	it.each([
		{ name: 'user', tier: TIER.user, schema: User.schema, keep: KEEP_USER },
		{ name: 'shopOwner', tier: TIER.shopOwner, schema: ShopOwner.schema, keep: KEEP_SHOP_OWNER }
	])('$name — every path is overwritten, removed or deliberately kept', ({ tier, schema, keep }) => {
		const scrub = buildAccountScrub(tier, ACCOUNT_ID, AT)
	...
	it.each([
		{ name: 'user', tier: TIER.user, schema: User.schema },
		{ name: 'shopOwner', tier: TIER.shopOwner, schema: ShopOwner.schema }
	])('$name — every member of login is overwritten or removed, none is merely left', ({ tier, schema }) => {
		const scrub = buildAccountScrub(tier, ACCOUNT_ID, AT)
	...
	it.each([
		{ name: 'user', tier: TIER.user, fields: ENCRYPTED_FIELDS_USER },
		{ name: 'shopOwner', tier: TIER.shopOwner, fields: ENCRYPTED_FIELDS_SHOP_OWNER }
	])('$name — no encrypted path survives a scrub untouched', ({ tier, fields }) => {
		const scrub = buildAccountScrub(tier, ACCOUNT_ID, AT)
...
describe('what the scrub writes', () => {
	it('moves the address to a per-account name under a host that can never resolve', () => {
		...
		expect(buildAccountScrub(TIER.user, ACCOUNT_ID, AT).$set['login.email']).toBe(scrubbedEmail(ACCOUNT_ID))
	})
	...
	it('leaves a password nobody holds, in the exact shape the validator demands', () => {
		...
		expect(buildAccountScrub(TIER.shopOwner, ACCOUNT_ID, AT).$set['login.password']).toBe(SCRUBBED_PASSWORD_HASH)
	})

	it('stamps scrubbedAt with the moment it was handed, so the sweep stops selecting the row', () => {
		expect(buildAccountScrub(TIER.user, ACCOUNT_ID, AT).$set.scrubbedAt).toBe(AT)
	})

	it('gives the customer the two-member block that collection requires, and nothing more', () => {
		expect(buildAccountScrub(TIER.user, ACCOUNT_ID, AT).$set.personalData).toStrictEqual({
	...
	it('gives the shop owner every member its all-or-nothing block requires', () => {
		expect(buildAccountScrub(TIER.shopOwner, ACCOUNT_ID, AT).$set.personalData).toStrictEqual({
			firstName: SCRUBBED_FIRST_NAME,
			lastName: SCRUBBED_LAST_NAME,
	...
	it('removes the address book only on the collection that has one, and the notes only on the other', () => {
		const user = buildAccountScrub(TIER.user, ACCOUNT_ID, AT).$unset
		const shopOwner = buildAccountScrub(TIER.shopOwner, ACCOUNT_ID, AT).$unset
	...
	it('removes the two credential sub-documents and the operator reason on both', () => {
		for (const tier of [TIER.user, TIER.shopOwner] as const) {
			const { $unset } = buildAccountScrub(tier, ACCOUNT_ID, AT)
	...
	it('keeps the closure record itself out of both halves of the update', () => {
		for (const tier of [TIER.user, TIER.shopOwner] as const) {
			const { $set, $unset } = buildAccountScrub(tier, ACCOUNT_ID, AT)
