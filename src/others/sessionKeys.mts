 ...
 *
 * ⚠️ **`incr` left this interface with the dual-read fallback.** It was here for one counter,
 ...
 * one, reusing one — would put the token back into a key name that is not a digest, undoing the hashed
 * namespace through the side door.
 */
export const familyKey = (familyId: string) => `${process.env.REDIS_KEY}family:${familyId}`

/**
 * The key of the hash naming every live session of one account — the shared prefix, the word
 ...
 * `user` who collided would share one index, and revoking one account's sessions would log out a stranger.
 *
...
/**
 * The session key one index field names — the shared prefix and the field, verbatim.
 *
 ...
 *
 * ⚠️ **Every session key on the platform has this one shape.** The raw-token shape this note
...
/**
 * The key of the list holding one account's reuse events — the shared prefix, the word `reuse:`,
 ...
 * are these" (BCON-08). The scope question was answered "per account only", and this key shape is that
...
/**
 * The counter incremented whenever a refresh loses a race and is told to retry.
 *
 ...
 * the next review of the grace window is what reads it.
 */
 ...
 *
 * ⚠️ **This is the detection that did not exist**, and the reason an open question stood through two
 ...
 * `keygripStatus` and for an admin with `redis-cli`.
 */
 ...
 *
 * ⚠️ **This read had a raw-key second half until the fallback was removed.** A token that misses the digest
 ...
 *
 * ⚠️ **This deleted two key shapes and now deletes one.** The second `del` was aimed at a
 ...
 * digest grants nothing, and the admin session console renders none of it.
 *
 ...
 * the value exists only to describe it to a human — the admin session console renders this list to an
 * account. Anything that could be presented as a credential belongs in the session hash, which is keyed by
 * a digest and read only by whoever already holds the token.
 *
...
/**
 * How long an index key lives: the **longer** of the two session caps, in seconds.
 *
 ...
 * session — live, listed nowhere, and therefore silently missed by any revocation. Per-field TTLs prune
 ...
 * scanning the keyspace. Called by every login and by every rotation.
 *
 * ⚠️ **The token goes in prefixed — `refresh:…`, exactly as `sessionKey` takes it.** The field name is
 * the digest of the *session key body*, which is what lets a revocation rebuild the key to delete as
 ...
 *
 * ⚠️ **The field carries its own TTL, and it is the session's absolute cap.** Rotation and
 ...
 * `originalLogin + sessionCapDays`, not the session key's 90-day physical TTL: under an absolute cap a session
...
/**
 * Removes one session from its account's index. Called by rotation for the token it has just
