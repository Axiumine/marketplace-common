 ...
 *
 * `notes` is free text an admin wrote *about* a named person, encrypted at rest and the one
 ...
 * still admin-*written*, but the two authorization gates now read it, so a list that forbade
 ...
 * on the `user` collection is real, but `user` carries no admin-only field today — no `notes`, no
 ...
 */
export const ADMIN_ONLY_FIELDS_SHOP_OWNER = ['notes'] as const

/**
 * The one admin-written `shopOwner` field the BC-01 authorization services must **read**.
 *
 * `waitApprov` is BC-03's manual-approval gate: an admin raises it to park an account pending
 ...
 *
 * It is kept out of `ADMIN_ONLY_FIELDS_SHOP_OWNER` because that list feeds the projection tests,
