/**
 * Why a lineage was revoked. Two values, one per place `revokeSessionFamily` is called from,
 ...
 * the other drift the moment a third case is added, and the drift shows up as an admin reading a blank
 * cell rather than as a failing build. `marketplace-admin` consumes this package, so its generated enum
 * can be asserted against this constant.
 *
 * ⚠️ **An admin's own revocation is deliberately not one of these.** Attributing a revoke to the Admin
 * who pressed the button is a second audit trail with its own retention question, and that question is
 * answered "not attributable" for now. Adding a value here would answer it by accident.
 */
